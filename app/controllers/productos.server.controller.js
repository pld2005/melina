...
exports.list = function(req, res) { 
	Producto.find({'user': req.user}).sort('name').populate('tipoiva').populate('tipoconcepto').populate('unidadmedida').populate('user', 'displayName').exec(function(err, productos) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
